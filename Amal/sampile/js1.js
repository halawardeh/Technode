...
fetch("JSON1.JSON")
  ...
  .then(data => {
    quizData = data;                   //these data(json file) store in qd
    loadQ();                          //*****function 1******** */
});
...
function loadQ() {
  ...
  CHOI.innerHTML = "";                     // Clear the previous choices if didnot do this thay well desplay uppove each other 
...
function ViewMore() {
    ...
    quizData.forEach((question, index) => {
      ...
      // first question is the object and the second is the properity or key for this
      result.innerHTML = `
